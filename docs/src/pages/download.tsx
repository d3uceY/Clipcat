...
const PLATFORMS = [
  ...
  },
  {
    ...
    variants: [
      {label: 'Linux (x64) Binary', href: `${RELEASES_BASE}/Clipcat-linux-amd64`, note: 'Standalone'},
...
function DownloadCard({
  ...
  return (
    <article className={clsx(styles.card, 'dl-card')}>
      ...
      {warning && (
        <div className={styles.warningBox}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={styles.warningIcon} aria-hidden="true">
          ...
          <div>
            <span>{warning}</span>
            <Link to={guideHash} className={styles.warningLink}>
              First-run guide →
            </Link>
          </div>
        </div>
...
export default function DownloadPage(): ReactNode {
  ...
  useGSAP(
    () => {
      ...
      mm.add('(prefers-reduced-motion: no-preference)', () => {
        ...
        tl.from('.dl-line-inner', {yPercent: 112, duration: 1.05, stagger: 0.12}, 0.05)
          .from('.dl-sub', {y: 22, opacity: 0, duration: 0.6}, '-=0.5')
          .from(
            '.dl-card',
            {y: 34, opacity: 0, duration: 0.7, ease: 'power3.out', stagger: 0.12, clearProps: 'transform'},
            '-=0.25',
          )
          .from('.dl-note', {opacity: 0, duration: 0.5}, '-=0.2');
      });
    },
    {scope},
  );
  ...
  return (
      <Layout title="Download Clipcat" description="Download Clipcat for Windows, macOS, and Linux. Free, open source, no account.">
      <main ref={scope} className={styles.page}>
        <div className={styles.hero}>
          <Heading as="h1" className={styles.title}>
            <span className={styles.titleMask}>
              <span className={clsx(styles.titleInner, 'dl-line-inner')}>Download</span>
            </span>
            <span className={styles.titleMask}>
              <span className={clsx(styles.titleInner, styles.titleAccent, 'dl-line-inner')}>Clipcat.</span>
            </span>
          </Heading>

          <div className={clsx(styles.versionBadge, 'dl-sub')}>
            {loading ? (
              <span className={styles.versionSkeleton} aria-label="Loading latest version" />
            ) : (
              <>
                <a
          ...
          <p className={clsx(styles.subtitle, 'dl-sub')}>
        ...
        <div className={styles.cards}>
          {PLATFORMS.map((p) => (
            <DownloadCard key={p.os} {...p} />
          ))}
        </div>

        <p className={clsx(styles.sourceNote, 'dl-note')}>
