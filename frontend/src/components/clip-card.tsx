...
function ClipCard({
  ...
  const labelSuggestions = useMemo(() => {
    ...
    return distinctLabels
      .filter(
        (l) =>
          l !== (clip.label || "") && (q === "" || l.toLowerCase().includes(q)),
      )
      .slice(0, 8);
    ...
    >
      ...
      <div className="mb-3 flex items-start justify-between">
        ...
        <span className="text-xs text-muted-foreground md:hidden flex items-center gap-1">
          {clip.source === "network" && <Wifi className="h-3 w-3" />}
          {relativeTime}
        </span>
      </div>
      ...
      <div
        className={`grid transition-[grid-template-rows] duration-200 ease-in-out${isEditingLabel || clip.label ? " grid-rows-[1fr]" : " grid-rows-[0fr] group-hover:grid-rows-[1fr]"}`}
      >
      ...
      {/* Content */}
      <div
        className={`mb-4 flex-1 overflow-hidden cursor-pointer hover:scale-95 transition-transform  ${hideContent ? "hard-to-read" : ""}`}
        onClick={(e) => {
          handleLinkClick(e);
      ...
      <div className="flex flex-col-reverse gap-2 justify-between">
        <span className="hidden text-xs text-muted-foreground md:block flex items-center gap-1">
          {clip.source === "network" && <Wifi className="h-3 w-3 inline" />}
          {relativeTime}
        </span>
