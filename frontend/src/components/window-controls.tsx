...
import { RefreshCw, Download, Wifi, Eye, EyeOff } from "lucide-react";
...
export default function WindowControls({
  ...
  // Keep local passphrase input in sync with loaded settings.
  useEffect(() => {
    setSyncPassphrase(syncSettings.passphrase);
  }, [syncSettings.passphrase]);

  const handleQuickPasteToggle = async () => {
    if (isQuickPaste) {
      playSound(
        isQuickPaste ? "/sounds/switch-on.mp3" : "/sounds/switch-off.mp3",
        soundOn,
        1,
      );
  ...
  const MenuSwitch = (
    ...
    const handleToggleFunction = () => {
      playSound(
        isOn ? "/sounds/switch-on.mp3" : "/sounds/switch-off.mp3",
        soundOn,
        1,
      );
      if (!disabled) {
  ...
  useGSAP(() => {
    gsap.set(settingDialogRef.current, {
      display: "none",
    });
  }, []);
  ...
  const handleSettingsClick = () => {
    ...
    if (!dialogOpen) {
      ...
      setDialogOpen(false);
      tl.to(settingDialogInnerRef.current, {
        opacity: 0,
        y: 10,
        rotation: 2,
        scale: 0.95,
        ...
        .set(settingDialogInnerRef.current, {
          rotation: 0,
          scale: 1,
        });
    }
    ...
    <div className="flex flex-row-reverse items-center fixed z-10 top-0 right-0 md:mr-[2%] md:pt-3 pt-2 mr-2 gap-6">
      <div className="mt-1 relative z-10">
        ...
        >
          ...
          />
          <div
            ref={settingDialogInnerRef}
            className="setting-dialog relative z-10 w-[90vw] max-w-sm rounded-sm border-0 h-screen! sm:h-[90vh]! max-h-100"
          >
            <button
              onClick={handleSettingsClick}
            ...
            <ScrollArea className="relative z-[1] h-full pt-6 px-6">
              ...
              {/* ── Window ── */}
              <p className="text-[10px] uppercase tracking-widest opacity-40 mt-1 mb-1">
                Window
              </p>
              <div
                className="flex items-center gap-3 justify-between py-2"
                title="alt + m to toggle"
              >
              ...
              <div
                ...
                title={
                  isQuickPaste
                    ? "Turn off Quick Paste to use Always on Top"
                    : "Keep the Clipcat window always above other windows"
                }
              >
                <div className="flex flex-col">
                  <p className="sm:text-base text-sm p-0!">Always on Top</p>
                  {isQuickPaste && (
                    <p className="text-[10px] opacity-50 p-0!">
                      Turn off Quick Paste first
                    </p>
                  )}
                </div>
                <span className="flex-1 border-b border-dashed border-current opacity-20  mb-1 mx-1" />
              ...
              <div
                className="flex items-center gap-3 justify-between py-2"
                title="Move the window next to your cursor when Quick Paste summons it"
              >
                <div className="flex flex-col">
                  <p className="sm:text-base text-sm p-0!">Smart Position</p>
                  {!isQuickPaste && (
                    <p className="text-[10px] opacity-50 p-0!">
                      Requires Quick Paste
                    </p>
                  )}
                </div>
                <span className="flex-1 border-b border-dashed border-current opacity-20  mb-1 mx-1" />
              ...
              <div
                className="flex items-center gap-3 justify-between py-2"
                title="Pause clipboard capture temporarily"
              >
              ...
              <div
                className="flex items-center gap-3 justify-between py-2"
                title="Limits the number of clipboard items stored"
              >
              ...
              <div
                className="flex items-center gap-3 justify-between py-2"
                title="Automatically hide clipboard items that look like passwords, API keys, or tokens"
              >
              ...
              <div
                className="flex items-center gap-3 justify-between py-2"
                title="Enables or disables loading the app on system startup"
              >
              ...
              <div
                className="flex items-center gap-3 justify-between py-2"
                title={`Quick Paste: hides to the tray, ${platform === "darwin" ? "Cmd" : "Ctrl"}+Shift+V summons it, paste any clip into the last window you used`}
              >
              ...
              {updateAvailable ? (
                <div className="py-2 space-y-2">
                  ...
                  <button
                    onClick={() =>
                      BrowserOpenURL(
                        "https://d3ucey.github.io/Clipcat/download",
                      )
                    }
              ...
              <div className="py-2">
                ...
                <div className="flex gap-1 mb-2">
                  <input
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && handleAddIgnoreEntry()
                    }
              ...
              <div
                className="flex items-center gap-3 justify-between py-2"
                title="Share clipboard content with other Clipcat instances on the same network"
              >
                <div className="flex flex-col">
                  <p className="sm:text-base text-sm p-0!">Sync Enabled</p>
                  {syncSettings.enabled && (
                    <p className="text-[10px] opacity-50 p-0! flex items-center gap-1">
                      <Wifi className="h-3 w-3" />
                      {syncSettings.peerCount} device
                      {syncSettings.peerCount !== 1 ? "s" : ""} connected
                    </p>
                  )}
                </div>
                <span className="flex-1 border-b border-dashed border-current opacity-20 mb-1 mx-1" />
                {MenuSwitch(syncSettings.enabled, async () => {
                  // Toggle: if turning off, save with enabled=false
                  // if turning on, save with current passphrase
                  await saveSyncSettings(!syncSettings.enabled, syncPassphrase);
                })}
              </div>
              <div className="py-2">
                <p className="text-xs text-foreground/60 mb-2">
                  Devices must share the same passphrase to sync. Data is
                  encrypted and sent only over your local network.
                </p>
                <div className="flex gap-1 mb-2">
                  <div className="relative flex-1">
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" &&
                        saveSyncSettings(syncSettings.enabled, syncPassphrase)
                      }
                      placeholder={
                        syncSettings.enabled
                          ? "Change passphrase…"
                      ...
                      className="w-full text-xs px-2 py-1 pr-7 border-b border-current bg-transparent focus:outline-none placeholder-gray-400"
                    />
                    <button
        ...
        >
          <button onClick={() => WindowMinimise()}>
            <img
              src="/minimize.png"
              alt="minimize"
              className="h-5 shadow-md/30"
            />
          </button>
          <button onClick={() => handleWindowScreen()}>
            <img
              src={fullScreen ? "/unmaximise.png" : "/maximize.png"}
              alt="maximize"
              className="h-5 shadow-md/30"
            />
          </button>
      ...
      <Dialog
        open={showQuickPasteConfirm}
        onOpenChange={(open) => {
          if (!open) setShowQuickPasteConfirm(false);
        }}
      >
        <DialogContent
              ...
              <ul className="text-sm mt-1 space-y-1.5">
                <li>
                  <span className="font-semibold">
                    {platform === "darwin" ? "Cmd" : "Ctrl"}+Shift+V
                  </span>{" "}
                  - summon Clipcat from any window
                </li>
                <li>
                  Click the content on a clip to fire it into the last window
                  you used
                </li>
                <li>
                  The tray icon also brings Clipcat back whenever you need it
